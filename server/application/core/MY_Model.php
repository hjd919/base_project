...
class BF_Model extends CI_Model
{
     ...
     */
    protected $created_field = 'create_time';

    /**
     * @var string Field name to use for the modified time column in the DB
     * table if $set_modified is enabled.
     */
    protected $modified_field = 'modify_time';
    ...
    public function __construct()
    {
        ...
        // observer system.
        if (true === $this->set_created) {
            array_unshift($this->before_insert, 'created_on');
        }
        if (true === $this->set_modified) {
     ...
     * 集成CI控制器的方法
     * @Author hnatao
     * @DateTime 2015-11-17T17:57:18+0800
     */
    ...
    public function insert($data = null)
    {
        ...
        if (true === $this->set_created && true === $this->log_user && !array_key_exists($this->created_by_field, $data)
        ) {
    ...
    public function insert_batch($data = null)
    {
        ...
        if (true === $this->set_created && true === $this->log_user) {
    ...
    {
        if ($data && false === $this->skip_validation) {
        ...
        if (true === $this->set_modified && true === $this->log_user && !array_key_exists($this->modified_by_field, $data)
        ) {
            $data[$this->modified_by_field] = $this->auth->user_id();
        }

        if (($result = $this->db->update($this->table_name, $data, $where)) !== false) {
    ...
    public function update_batch($data = null, $index = null)
    {
        ...
        if (true === $this->set_modified && !array_key_exists($this->modified_field, $data)) {
            foreach ($data as $key => $record) {
                $data[$key][$this->modified_field] = $this->set_date();
                if (true === $this->log_user && !array_key_exists($this->modified_by_field, $data[$key])) {
    ...
    public function get_validation_rules($type = 'update')
    {
        ...
        if ('insert' == $type && is_array($this->insert_validation_rules)
    ...
    protected function _return_type($multi = false)
    {
        ...
        return 'array' == $this->temp_return_type ? "{$method}_array" : $method;
    }
    ...
    {
        if ('int' != $format && 'datetime' != $format && 'date' != $format) {
            return false;
        }
    ...
    public function set_modified($modified = true)
    {
        ...
        if (false === $modified || true === $modified) {
